...
export class MaintenanceService {
  static async createMaintenance(data: MaintenanceInput): Promise<MaintenanceResponse> {
    ...
    const newMaintenanceJob = await prisma.job.create({
      data: {
        ...
        vehicle_number,
        golf_course_id: golf_course_id.toString(),
  ...
  static async getAllMaintenance(): Promise<MaintenanceResponse[]> {
    ...
    return maintenanceItems.map((item: any) => this.formatMaintenanceResponse(item));
  }
  ...
  static async updateMaintenance(id: string, data: Partial<MaintenanceInput>): Promise<MaintenanceResponse | null> {
    ...
    if (data.vehicle_id) updateData.vehicle_id = data.vehicle_id.toString();
    if (data.vehicle_number) updateData.vehicle_number = data.vehicle_number;
    if (data.golf_course_id) updateData.golf_course_id = data.golf_course_id.toString();
    if (data.user_id) updateData.user_id = data.user_id.toString();
    if (data.userName) updateData.userName = data.userName;
    if (data.date) updateData.createdAt = new Date(data.date);
